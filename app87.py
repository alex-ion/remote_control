...
from frames import Timer, Hands
...
class PomodoroTimer(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        timer_frame = Timer(container, self, lambda: self.show_frame(Hands))
        timer_frame.grid(row=0, column=0, sticky = "NESW")

        sliders_frame = Hands(container, self, lambda: self.show_frame(Timer))
        sliders_frame.grid(row=0, column=0, sticky = "NESW")

        self.frames[Timer] = timer_frame
        self.frames[Hands] = sliders_frame
